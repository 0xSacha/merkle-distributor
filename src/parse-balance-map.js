...
function parseBalanceMap(balances) {
  ...
  var balancesInNewFormat = Array.isArray(balances)
    ...
    : Object.keys(balances).map(function (account) {
        return {
          address: account,
          earnings: '0x'.concat(balances[account].toString(16)),
          reasons: '',
        }
      })
  var dataByAddress = balancesInNewFormat.reduce(function (memo, _a) {
    ...
    if (memo[parsed]) throw new Error('Duplicate address: '.concat(parsed))
    var parsedNum = ethers_1.BigNumber.from(earnings)
    if (parsedNum.lte(0)) throw new Error('Invalid amount for account: '.concat(account))
    var flags = {
  ...
  // generate claims
  var claims = sortedAddresses.reduce(function (memo, address) {
    var _a = dataByAddress[address],
      amount = _a.amount,
      flags = _a.flags
    memo[address] = __assign(
      { amount: amount.toHexString(), proof: tree.getProof(address, amount) },
      flags ? { flags: flags } : {}
    )
    return memo
  }, {})
